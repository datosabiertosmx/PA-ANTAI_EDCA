...
router.get('/', isNotAuthenticated, function (req, res, next) {
    res.render('index', {title: 'Sistema de captura de datos de contrataciones abiertas en Panamá', message: req.flash('message')});
});
...
router.get('/main', isAuthenticated, function(req, res, next) {
    res.render('main', { user: req.user, title: 'Sistema de captura de datos de contrataciones abiertas en Panamá' });
});
    ...
    query.then(function (contratacion) {
        ...
        // using .spread(function(user, event)) is best here, if supported;
            .then(function (data) {
                ...
                res.render('main', {
                    user: req.user,
                    title: 'Sistema de captura de datos de contrataciones abiertas en Panamá',
                    cp: data[0],
                    planning: data[1] || {},
                    budget: data[2] || {},
                    tender: data[3] || {},
...
router.post('/update-contract', isAuthenticated, async function (req, res) {
    ...
    " datesigned=$11, amendment_date=$12, amendment_rationale=$13, value_amountnet=$14, surveillanceMechanisms=$15, datelastupdate = now()" +
    " where ContractingProcess_id = $1  and id = $16  returning id", [
        req.body.contractingprocess_id,
        req.body.contractid,
        req.body.awardid,
        req.body.title,
...
router.post('/new-budgetbreakdown', isAuthenticated, function (req, res) {
    let query = `insert into $1~ (budgetbreakdown_id, description, amount, currency, url, budgetbreakdownPeriod_startdate, budgetbreakdownPeriod_enddate, source_id, contractingprocess_id)
...
router.post('/newbudgetbreakdown-fields', isAuthenticated, function (req, res) {
    db_conf.edca_db.task(function (t) {
        return t.batch([
            t.manyOrNone("select distinct currency, alphabetic_code from currency order by currency"),
...
router.post('/edit-budgetbreakdown', isAuthenticated, async function (req, res) {
    ...
    budgetbreakdownPeriod_enddate = $9, source_id = $10 where id = $2 returning id`;
...
router.post('/editbudgetbreakdown-fields', isAuthenticated, function (req, res) {
    db_conf.edca_db.task(function (t) {
        return t.batch([
...
router.post('/search-contractingprocess', function (req, res) {
    db_conf.edca_db.task(function (t) {
        return this.batch([
            this.manyOrNone("select *, concat($2, '/record-package/', ocid) as record from contractingprocess where ocid like '%' || $1 || '%' order by ocid limit 7", [req.body.search, getHost(req)])
        ]);
    }).then(function (data) {
        res.json(data[0]);
    }).catch(function (error) {
        ...
        if (req.body.stage === 'planning'){
          ...
          db_conf.edca_db.one('update award set awardid = $2, title = $3, description = $4, status = $5, award_date = $6, value_amountnet = $7, value_amount = $8, value_currency = $9 where contractingprocess_id = $1  and id = $10 returning id as award_id',
            ...
            db_conf.edca_db.one('update contract set awardid =$2, contractid = $3 ,title = $4, description=$5, status = $6, period_startdate=$7, period_enddate=$8, value_amountnet=$9, value_amount=$10,' +
                ' datesigned=$11 where id = $1 returning id as contract_id',
                [
                    req.body.localid,
                    jsonArray[0].IDENTIFICADOR_ADJUDICACION,
    ...
    db_conf.edca_db.tx(function (t) {
        return t.batch([
            ...
            this.one('update roles set buyer=$2, procuringentity=$3, supplier=$4, tenderer=$5, guarantor=$6,' +
                ...
                'official=$12, invitedSupplier=$13, issuingSupplier=$14, requestingunit=$15, contractingunit=$16, technicalunit=$17  where parties_id = $1 returning id', [
                req.body.parties_id,
                isChecked(req.body.buyer),
                isChecked(req.body.procuringEntity),
                isChecked(req.body.supplier),
...
router.post('/validate-process/search', isAuthenticated, async  (req, res) => {
    try{
        ...
        let processes = await db_conf.edca_db.manyOrNone(`select id,
                                                                 ocid,
                                                                 publisher,
                                                                 updated,
                                                                 published,
                                                                 ...
                                                                 pnt_published,
                                                                 pnt_version,
                                                                 to_char(pnt_date, 'YYYY/MM/DD') as pnt_date  
                                                          from contractingprocess cp 
                                                          where id in (select contractingprocess_id from tender where tenderid is not null) 
                                                          ${(where.length > 0 ? ' AND ' + where.join(' AND ') : '')} order by id`, req.body);
