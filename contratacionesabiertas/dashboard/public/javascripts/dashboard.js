$(document).ready(function () {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    switch (data[i].procurementmethod_details) {
                        case 'Compra menor':
                            colors.push('#09b109');
                            break;
                        case 'Licitación por convenio marco':
                            colors.push('#00cc99');
                            break;
                        case 'Licitación pública':
                            colors.push('#ffcc00');
                            break;
                        case 'Licitación pública por mejor valor':
                            colors.push('gray');
                            break;
                        case 'Procedimiento especial':
                            colors.push('#663399');
                            break;
                        case 'Procedimiento excepcional':
                            colors.push('#ff6600');
                            break;
                        case 'Subasta en reversa':
                            colors.push('#00BFFF');
                            break;
                        case 'Subasta de bienes públicos':
                            colors.push('#ff008c');
                            break;
                    }
                }
    ...
    function stageChart() {
        $.post('/contratacionesabiertas/stage-chart-data', {
            ...
            var nData = [[],[],[],[],[]];
            var ticks = ['Compra menor', 'Licitación por convenio marco', 'Licitación pública', 'Licitación pública por mejor valor', 'Procedimiento especial', 'Procedimiento excepcional', 'Subasta en reversa', 'Subasta de bienes públicos'];
            var chartOptions = {
                height: 550,
                stackSeries: true,
                seriesDefaults: {
            ...
            function resizeHandler () {
                if (window.innerWidth < 700) {
                    chartOptions['axes']['yaxis']['ticks'] = ['CM', 'LP', 'A41', 'A42', 'ER', 'I3P', 'CC', 'AM'];

                    plot.destroy();
                    plot = ff('stages_chart', bData, chartOptions);
                } else {
                    chartOptions['axes']['yaxis']['ticks'] = ['Compra menor','Licitación por convenio marco', 'Licitación pública', 'Licitación pública por mejor valor', 'Procedimiento especial', 'Procedimiento excepcional', 'Subasta en reversa', 'Subasta de bienes públicos']; //procedimiento por etapas
...
function drawSeriesChart() {
    ...
    $.post('/contratacionesabiertas/bubble-chart-data', {
        ...
        var newData = [['ID', 'Fecha de firma', 'Vigencia (días naturales)', 'Tipo', 'Monto USD']];
        ...
        if (data.length > 0) {
            for (i = 0; i < data.length; i++) {
                newData.push([ data[i].title , new Date(data[i].datesigned), Math.abs(data[i].vigencia.days), data[i].procurementmethod_details, Number(data[i].value_amount)]);
            }
        } else {
            return;
        }
        ...
        var options = {
          ...
          // Gráfica Contrataciones en el tiempo
            	'Compra menor': {color: '#09b109'},
                'Licitación por convenio marco': {color: '00cc99'},
                'Licitación pública': {color: '#ffcc00'},
                'Licitación pública por mejor valor': {color: 'gray'},
                'Procedimiento especial': {color: '#663399'},
                'Procedimiento excepcional': {color: '#ff6600'},
                'Subasta en reversa': {color: '#00BFFF'},
                'Subasta de bienes públicos': {color: '#ff008c'}
            },
            legend: {
                position: 'bottom',
                textStyle: {
