...
router.get('/', function(req, res, next) {
    ...
    edca_db.task(function (t) {
        ...
        this.one('select count(*) as total from contractingprocess, contract where contractingprocess.id = contract.contractingprocess_id and contract.value_amount > 0 '),
        this.one('select sum(contract.value_amount) as total from contractingprocess, contract where contractingprocess.id = contract.contractingprocess_id ')
        ]);
    }).then(function (data) {
...
router.get('/contratos/:year?',function (req, res) {
    ...
    edca_db.task(function (t) {
        return this.batch([
            ...
            this.one('select count(*) as total from contractingprocess, contract where contractingprocess.id = contract.contractingprocess_id and contract.value_amount > 0' + where),
            this.one('select sum((select sum(value_amount) from contract where contractingprocess_id = contractingprocess.id)) as total from contractingprocess where 1 = 1' + where2),
            this.manyOrNone(`select t.procurementmethod_details, count(*) as conteo, sum(t.total) as total
                from (select tender.procurementmethod_details,
                    (select sum(value_amount) from contract where contractingprocess_id = contractingprocess.id) as total
...
router.post('/pagination', function (req, res) {
    ...
    edca_db.task(function (t) {
        ...
        var query1 = `select * from (select contractingprocess.id as localid, contractingprocess.uri, contractingprocess.ocid, contractingprocess.stage, contractingprocess.description,
            ...
            (select sum(value_amount) from contract where contractingprocess_id = contractingprocess.id) as value_amount,
        ...
        if (req.body.orderby === undefined) {
            req.body.orderby = 'value_amount';
        }

        query1 += " order by $6~" + (req.body.orderby === 'value_amount' || req.body.orderby === 'datesigned' ? " asc" : "");
...
router.post('/donut-chart-data', function (req, res) {
    ...
    from (select tender.procurementmethod_details,
        (select sum(value_amount) from contract where contractingprocess_id = contractingprocess.id) as sum,
        (select sum(value_amount) from contract where contractingprocess_id = contractingprocess.id)/(select sum(value_amount) from contract) * 100 as percentage
        from contractingprocess
        inner join tender on tender.contractingprocess_id = contractingprocess.id
